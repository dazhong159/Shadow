...
//    imageLoader.displayImage(adLogoView, url);
  ...
  private static LogoImageView displayLogoView(Context context,
    ...
    if (adLogoParams == null) {
      int width = defaultWidth < 0 ? defaultWidth : (int) PxUtils.dpToPx(context, defaultWidth);
      int height = defaultHeight < 0 ? defaultHeight : (int) PxUtils.dpToPx(context, defaultHeight);
      adLogoParams = new FrameLayout.LayoutParams(width, height);
      adLogoParams.gravity = Gravity.RIGHT | Gravity.BOTTOM;
    }
